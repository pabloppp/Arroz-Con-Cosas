...
checkpoint_path = "../../models/arroz_con_cosas/prior_v1.pt"

wandv_project = "ArrozConCosas"
wandv_entity = "babbleberns"
wandb_run_name = "prior_v1"
...
def ddp_setup(rank, world_size, n_node, node_id): # <--- DDP
    ...
    init_process_group(
        ...
        init_method="file:///fsx/home-pablo/src/arroz_con_cosas/dist_file_prior_v1",
    )
...
def train(gpu_id, world_size, n_nodes):
    ...
    # - denoisegic - 
    model = PriorModel().to(device)
    model_ema = PriorModel().to(device).eval()
    ...
    for it in pbar:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                image_embeddings = clip_model.encode_image(images).float()
        ...
        del pred_noise, images, image_embeddings, text_embeddings, t, noised_image_embeddings, noise, loss, loss_adjusted
        torch.cuda.empty_cache()
